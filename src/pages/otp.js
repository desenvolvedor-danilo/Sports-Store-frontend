...
export default function Otp(){
  ...
  useEffect(()=>{
    setEmailStored({email:window.localStorage.getItem("email")})
  },[])
  ...
  const verifyOtp = (e) =>{
    e.preventDefault();
    fetch(`http://localhost:8080/user/email?code=${parseInt(inputOtp.join("")).toString()}`)
    .then((res)=>{
      if(res.status===202){
        ...
        <div className="fadeInDownOtp">
            <div id="formContentOtp">
              <div className="fadeIn first">
                <h1 id="recovery">Recuperar senha</h1>
              </div>
